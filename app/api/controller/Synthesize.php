...
class Synthesize extends BaseController
{
    ...
    public function leaderScore(){
        ...
        try {
            validate(Validate::class) -> scene('leader_score') -> check($data);
        }catch (\Exception $exception){
            return $this -> fail($exception -> getMessage());
        }
        $this -> business -> leaderScore($data);
    ...
    public function getPoorScore(){
        ...
        return $this -> success($this -> business -> getPoorScore($uid, $id));
    }
    ...
    public function downloadProve() {
        $data['uid'] = $this -> getParamUid();
        $data['target'] = $this -> request -> param("target", '', 'htmlspecialchars');
        try {
            validate(Validate::class) -> scene('download_prove') -> check($data);
        }catch (\Exception $exception){
            return $this -> fail($exception -> getMessage());
        }
        $this -> business -> downloadProve($data);
    }
}
